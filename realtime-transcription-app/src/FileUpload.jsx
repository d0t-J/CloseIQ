...
function FileUpload({ session, backendUrl }) {
  ...
  const loadUploadedFiles = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${backendUrl}/user/${session.user.id}/files`);
  ...
  const handleFileChange = (e) => {
    ...
    if (remainingSlots <= 0) {
      setMessage(`❌ Maximum ${maxFiles} files allowed. Please delete a file first.`);
      setFile(null);
      return;
    }
    ...
    const fileExists = uploadedFiles.some(f => f.filename === selectedFile.name);
    if (fileExists) {
      setMessage(`❌ File "${selectedFile.name}" already exists. Please delete it first or rename your file.`);
      setFile(null);
      return;
    }
  ...
  const handleDelete = async (filename) => {
    if (!confirm(`Are you sure you want to delete "${filename}"? This will remove the file and all its embeddings.`)) {
      return;
    }
    ...
    try {
      ...
      const response = await fetch(`${backendUrl}/user/${session.user.id}/files/${encodeURIComponent(filename)}`, {
        method: 'DELETE',
      });
    ...
    <div className="file-upload-container">
      ...
      <div className="file-limit-indicator">
        ...
        <p className="limit-text">
          {uploadedFiles.length}/{maxFiles} files used • {remainingSlots} slot{remainingSlots !== 1 ? 's' : ''} available
        </p>
      </div>
            ...
            <div key={index} className="file-item">
              <div className="file-info">
                ...
                <div className="file-details">
                  <span className="file-chunks">🧩 {fileInfo.chunks_count} chunks</span>
                  <span className="file-size">💾 {formatFileSize(fileInfo.file_size)}</span>
                  <span className="file-date">📅 {formatDate(fileInfo.upload_timestamp)}</span>
                </div>
              </div>
              <button
      ...
      {remainingSlots > 0 ? (
        ...
        <div className="no-slots-available">
          <p>⚠️ Maximum files reached. Delete a file to upload a new one.</p>
        </div>
      )}
